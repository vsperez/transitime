/**
 * 
 * This class is intended for relpacing the current implementation.
 * 
 */
...
//	happen to be drawn first.
	...
	//constructor(canvas,onclick,menu)
	{
		...
		L.tileLayer(params.mapTileUrl,
				// Specifying a shorter version of attribution. Original really too long.
				//attribution: 'Map data &copy; <a href="http://openstreetmap.org">OpenStreetMap</a> contributors, <a href="http://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>, Imagery © <a href="http://mapbox.com">Mapbox</a>',
				{attribution: '&copy; <a href="http://openstreetmap.org">OpenStreetMap</a> &amp; <a href="http://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>, Imagery © ' + params.mapTileCopyright,
			maxZoom: 19
				}).addTo(this.map);
		...
		//this.test=function(){alert("1313123")};
		//this.test();
		//TODO: ADD PARAMS TO SHOW WHAT WE WANT
		/////////////////////////////
		//FOR DRAWING
		...
		console.log(this.__onDrawCreated);
		this.__onDrawCreated=this.__onDrawCreated.bind(this);
		this.__onDrawStart=this.__onDrawStart.bind(this);
		console.log(this.__onDrawCreated);
		this.map.on('draw:created', this.__onDrawCreated);
		this.map.on('draw:drawstart', this.__onDrawStart);
		...
		////////////////////////////////
		//L.map('map',params);
		//this.createMap(params.mapTileUrl,params.mapTileCopyright);
		//this.editableLayers=null;
		this.createUserLocationMarker=this.createUserLocationMarker.bind(this);
		//this.editableLayers = this.editableLayers.bind(this);
		this.getEditableLayers = this.getEditableLayers.bind(this);
...
//		Globals are set via showRoute()
		...
		var apiUrlPrefix;
		this.initialStopId; // So can popup predictions window at startup
		var initialRouteId;
...
//         });
	...
	return;
	__onDrawCreated(e) {
		console.log( this.editableLayers.getLayers().length);
		console.log( this.drawControl);
		//console.log($(".leaflet-draw-draw-polyline").addClass("leaflet-disabled"));
	 ...
	 */
	 ...
	 predictionCallback(preds, status) {
	 ...
	 */
	getAndShowPredictionsForStop(routeShortName, stopId) {
	...
	zoomIn() {
		...
		var bounds = [];
		bounds.push(L.latLng(this.initialStop.lat, this.initialStop.lon));
	 ...
	 */
	 ...
	 showRouteInMap(routesData,specificDirection) {
		...
		// the paths & stops are drawn below the vehicles even if the vehicles
		// happen to be drawn first.
		this.routeFeatureGroup = L.featureGroup();
		...
		for (var i=0; i<route.direction.length; ++i) {
			...
			if(specificDirection  && direction.id!=specificDirection)
				continue;
			for (var j=0; j<direction.stop.length; ++j) {
				...
				var stopMarker = L.circleMarker([stop.lat,stop.lon], options).addTo(this.map);

				this.routeFeatureGroup.addLayer(stopMarker);
		...
		for (var i=0; i<route.shape.length; ++i) {
			...
			if(specificDirection  && shape.directionId!=specificDirection)
				continue;
			...
			this.routeFeatureGroup.addLayer(polyline);
		}
		...
		this.map.fitBounds(locsToFit, {animate: false});
		...
		this.routeFeatureGroup.bringToBack();
		...
		if (this.userLatLng) {
			showWalkingDirections(this.userLatLng.lat, this.userLatLng.lng, 
					this.initialStop.lat, this.initialStop.lon);
		}

		// After a bit of a delay, once user can see context, zoom in further
		//TODO:
		//setTimeout(this.zoomIn, 1200);
	}
	 ...
	 */
	 formatSpeed(speedInMetersPerSec) {
	 ...
	 */
	 getVehiclePopupContent(vehicleData) {
	 ...
	 */
	 getVehicleMarkerBackgroundOptions(vehicleData) {
	 ...
	 */
	 vehicleLocationsCallback(vehicles, status) {
	 ...
	 */
	 interpolateVehicle(vehicleMarker, cnt, interpolationSteps, origLat, origLon, newLat, newLon) {
...
//		" interpolatedLat=" + interpolatedLat + " interpolatedLon=" + interpolatedLon);
	 ...
	 * to rounding.
	 */
	 animateVehicle(vehicleMarker, origLat, origLon, newLat, newLon) {
	 ...
	 */
	 showWalkingDirections(lat1, lon1, lat2, lon2) {
	 ...
	 * @param latlng
	 */
	 createUserLocationMarker(latlng, accuracyRadius) {
	 ...
	 */
	 setUserLocationMarkerLocation(latlng, accuracyRadius) {
		if (userMarker)
	 ...
	 */
	 locationFound(locationEvent) {
		...
		if (!this.userLatLng && this.initialStop) {
			this.showWalkingDirections(locationEvent.latlng.lat, locationEvent.latlng.lng, 
					initialStop.lat, initialStop.lon);
		}

		// Remember user location so can zoom to it when route selected
		this.userLatLng = locationEvent.latlng;

		if (this.userMarker) {
			// userMarker already created so move it to proper location
			this.setUserLocationMarkerLocation(locationEvent.latlng, locationEvent.accuracy);
		} else {
			// userMarker not yet created so create it now
			this.createUserLocationMarker(locationEvent.latlng, locationEvent.accuracy);
		}
